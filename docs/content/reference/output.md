...
describes what users get. Pick a format with `-o`, or let harrypotter choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
harrypotter <command> -o table   # aligned columns for reading
harrypotter <command> -o jsonl   # one JSON object per line, for piping
harrypotter <command> -o json    # a single JSON array
harrypotter <command> -o csv     # spreadsheet friendly
harrypotter <command> -o tsv     # tab-separated
harrypotter <command> -o url     # just the URL column
harrypotter <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
harrypotter <command> --fields id,title,url
```
...
```bash
harrypotter <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
harrypotter <command>            # a table, because this is a terminal
harrypotter <command> | wc -l    # JSONL, because this is a pipe
```
